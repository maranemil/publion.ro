...
class UsersController extends AppController {
	...
	function login() {
		...
		if(!empty($this->data)) {
			...
			if(!empty($someone['User']['password']) && $someone['User']['password'] == md5($this->data['User']['password']) && $someone['User']['active'] == "true") {
...
*/
	...
	/* myentries - get my entries by id - anunturi 
	--------------------------------------------------------------------- */
	 function myentries($id){
		$this->checkIfLogged();
	...
	--------------------------------------------------------------------- */
	 function getusernamebyid($id = NULL){
			...
			if($tmpUsr["User"]["name"])	return ucwords(strtolower($tmpUsr["User"]["name"]));
			else if($tmpUsr["User"]["pastname"])	return ucwords(strtolower($tmpUsr["User"]["pastname"]));
			else if($tmpUsr["User"]["nickname"])	return ucwords(strtolower($tmpUsr["User"]["nickname"]));
			//$this->set("commdet", $commdet);
	}


	/* myfavs - get my favs list
	--------------------------------------------------------------------- */
	 function myfavs($id){
		$this->checkIfLogged();
			...
			if(count($arArtIDs)>1)  $sArtList = substr($sArtList,0,strlen($sArtList)-1);
	...
	function myfriends($id){
		$this->checkIfLogged();
			...
			//echo $sFrList; die();
			if(count($arFrIDs)>1)
				$srFrIDs = substr($srFrIDs,0,strlen($srFrIDs)-1);
	...
	/* forgotpassword - Sent email with token
	--------------------------------------------------------------------- */
