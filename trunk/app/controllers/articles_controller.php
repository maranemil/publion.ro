...
class ArticlesController extends AppController
{
	...
	var $uses = array('Article','User','Category','Subcategory','Rating','Fav');
	...
	{
		$criteria = null;
		$criteria = " Article.category_id !='8' "; // exclude matrimoniale din index
		...
		if($listtype==1){		$paging['show']='10'; }
		else if($listtype==2){	$paging['show']='20'; }
		else if($listtype==3){	$paging['show']='10'; }
		...
		/*$arTmpArt = $this->Article->find(
			...
			'limit' => $limit, //int
			'page' => $page, //int
			'callbacks' => false //weitere mögliche Werte: false, 'before', 'after'
			)
		);*/
		...
		$arTmpCats = $this->Category->query($sSQLA); 
		
		$sSQLB = 'SELECT * FROM categories LEFT JOIN subcategories ON categories.id = subcategories.category_id ORDER BY categories.name ASC';
	...
	public function view($id = NULL) {
		...
		$update = $this->Article->query("UPDATE articles SET views = '".$viewsplus."' WHERE id=".$id);
	...
	*/
	...
	public function step2() {
		...
		*/
		...
		if ($this->data["images"]["File"]["tmp_name"]){
			...
			if(
				(strstr(strtolower($this->data['Article']['title']),"erotic"))||
				(strstr(strtolower($this->data['Article']['descr']),"erotic"))
			)
			{	// Massage Masaj Erotice
		...
		$form_data = array(
						'Article' => array(
						'category_id'		=>	$arCat[0], 
						'subcategory_id'	=>	$arCat[1], 
						'title'				=>	$this->data['Article']['title'], 
						'descr'				=>	$this->data['Article']['descr'], 
						'phone'				=>	$this->data['Article']['phone'], 
						'date'				=>	date("Y-m-d"), 
						'image'				=>	$output,
						'price'				=>	$this->data['Article']['price'], 
						'webpage'			=>	$this->data['Article']['webpage'], 
						'ip1'				=>	$REMOTE_ADDR,
	...
	{
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="cumparare") $subcatID = 1;
		if($subcat=="vanzare") $subcatID = 2;
	...
	public function calculatoare($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="componente")	$subcatID = 3;
		if($subcat=="servicii")		$subcatID = 4;
		if($subcat=="sisteme")		$subcatID = 5;
		if($subcat=="software")		$subcatID = 6;
	...
	public function constructii($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="materiale")	$subcatID = 7;
		if($subcat=="utilaje")		$subcatID = 8;
	...
	public function diverse($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="agricultura")		$subcatID = 9;
		if($subcat=="animale")			$subcatID = 10;
		if($subcat=="antichitati")		$subcatID = 11;
		if($subcat=="carti")			$subcatID = 12;
		if($subcat=="diverse")			$subcatID = 13;
		if($subcat=="hobby")			$subcatID = 14;
		if($subcat=="servicii")			$subcatID = 15;
	...
	public function electronice($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="audio_video")		$subcatID = 16;
		if($subcat=="telefoane")		$subcatID = 17;
		if($subcat=="uz_casnic")		$subcatID = 18;
	...
	public function funerare($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="decese")		$subcatID = 19;
		if($subcat=="servicii")		$subcatID = 20;
	...
	public function imobiliare($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="cumparari")		$subcatID = 21;
		if($subcat=="inchirieri")		$subcatID = 22;
		if($subcat=="schimburi")		$subcatID = 23;
		if($subcat=="vanzari")			$subcatID = 24;
	...
	{
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="barbati")		$subcatID = 25;
		if($subcat=="femei")		$subcatID = 26;
		if($subcat){
	...
	{
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="cereri")		$subcatID = 27;
		if($subcat=="oferte")		$subcatID = 28;
...
public function produse($subcat=NULL)
	{
		...
		$this->set('title_for_layout','Publion - Anunturi Gratuite - Timisoara - Vanzari - Cumparari - Anunturi cu Poza');
		if($subcat=="imbracaminte")		$subcatID = 29;
		if($subcat=="incaltaminte")		$subcatID = 30;
		if($subcat=="cosmetice")		$subcatID = 31;
...
*/
	...
	function searcharticle() 
	{
		...
		*/
		...
		if($cntwords>1){
			$arWords = explode(" ",$searchq);
			$searchqStr1 = $arWords[0];
			$searchqStr2 = $arWords[1];
	...
	function statistics($limit = 5){

		if($limit  > 10) $limit = 10;
		/* TOP 10 suer / anunturi
		------------------------------------------------*/
		$sSQL = "select user_id, count(*)as total from articles WHERE user_id != 0 group by user_id  ORDER BY total DESC LIMIT ".$limit."";
		...
		foreach($smonth as $distMonth){
			...
			foreach($arTmp as $sTmp){
				if($sTmp[0]["total"]==0) $sTmp[0]["total"] = 50;
...
/**
	...
	*/
	 ...
	 function getRandomArticleTags($limit = 10){

		//if(!is_numeric($limit)) $this->redirect('/');
		if( isset($this->params['requested']) AND $this->params['requested'] )
		{
			//return 	$this->Company->find('all', array('order' => 'Company.id DESC', 'limit' => $limit));
			return $this->Article->query("SELECT title FROM `articles` WHERE title != '' ORDER BY RAND() LIMIT ".$limit." ");
		}
		else
		{
			return FALSE;
	...
	function getlastarticlebyuser($iduser=70){
