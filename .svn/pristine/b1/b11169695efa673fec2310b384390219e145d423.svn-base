...
class SluggableBehavior extends ModelBehavior
{
	...
	function beforeSave(&$Model)
	{
		...
		if ( $Model->hasField($this->__settings[$Model->alias]['slug']) AND ($this->__settings[$Model->alias]['overwrite'] OR empty($Model->id)) )
		{
			...
			if ( ! empty($label) )
			{
				...
				if ( ! empty($Model->whitelist) AND ! in_array($this->__settings[$Model->alias]['slug'], $Model->whitelist) )
				{
	...
	{
		if ( ! empty($settings['translation']) AND is_array($settings['translation']) )
		{
			// Run user-defined translation tables

			if ( count($settings['translation']) >= 2 AND count($settings['translation']) % 2 == 0 )
			{
				for ( $i=0, $limiti = count($settings['translation']); $i < $limiti; $i += 2 )
				{
					...
					if ( is_string($from) AND is_string($to) )
					{
		...
		else if ( ! empty($settings['translation']) AND is_string($settings['translation']) AND in_array(low($settings['translation']), array('utf-8', 'iso-8859-1')) )
		{
